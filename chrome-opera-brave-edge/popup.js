...
function renderDiff(raw, status) {
  ...
  // Prvo preuzimanje - prikazi obe liste odjednom
  if (d.first) {
  ...
  // Nema promena ni u jednoj listi
  if (!botRow && !wlRow) {
  ...
  // Prikazi samo liste koje su se promenile
  const rows = [];
...
el('update').addEventListener('click', () => {
  ...
  try {
    chrome.runtime.sendMessage({ type: 'refreshNow' }, (res) => {
      btn.disabled = false;
      btn.textContent = 'Ažuriraj bazu';
      if (chrome.runtime.lastError) {
        el('msg').textContent = 'Greška: ' + chrome.runtime.lastError.message;
...
function exportList(key, filename) {
  ...
  try {
    chrome.runtime.sendMessage({ type: 'getList', key: key }, (res) => {
      // Prazna (validna) whitelist-a daje text === '' - to NIJE greska.
      if (chrome.runtime.lastError || !res || !res.ok || typeof res.text !== 'string') {
